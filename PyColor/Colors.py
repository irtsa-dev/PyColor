...
class RGB:
    ...
    def hsl(self) -> tuple:
        ...
        M = max([self.red, self.green, self.blue]) / 255
        m = min([self.red, self.green, self.blue]) / 255

        C = M - m
        L = (M + m) / 2

        S = 0
        if C != 0: S = ((C / (1 - abs((2 * L) - 1))) * 100)
    ...
    def percentForm(self) -> tuple:
        ...
        r = round((self.red / 255), 2)
        g = round((self.green / 255), 2)
        b = round((self.blue / 255), 2)
    ...
    def grayscale(self) -> tuple:
        ...
        gv = int(round((self.red + self.green + self.blue) / 3))
...
class HEX:
    ...
    def hsv(self) -> tuple:
        if not self.__valid: return None
	return RGB(self.rgb[0], self.rgb[1], self.rgb[2]).hsv
    

    @property 
    def hsl(self) -> tuple:
        if not self.__valid: return None
        return RGB(self.rgb[0], self.rgb[1], self.rgb[2]).hsl
    

    @property
    def xyz(self) -> tuple:
        if not self.__valid: return None
	return RGB(self.rgb[0], self.rgb[1], self.rgb[2]).xyz

      
    @property
    def ycc(self) -> tuple:
        if not self.__valid: return None
	return RGB(self.rgb[0], self.rgb[1], self.rgb[2]).ycc
      

    @property
    def cmyk(self):
        if not self.__valid: return None
	return RGB(self.rgb[0], self.rgb[1], self.rgb[2]).cmyk
    ...
    def percentForm(self) -> float:
        ...
        return round((number / 16777215), 3)
    ...
    def grayscale(self) -> tuple:
        ...
        gv = int(round((self.rgb[0] + self.rgb[1] + self.rgb[2]) / 3))
...
class HSV:
    ...
    def hexidecimal(self) -> str:
        if not self.__valid: return None
	return RGB(self.rgb[0], self.rgb[1], self.rgb[2]).hexidecimal
    ...
    def hsl(self) -> tuple:
        ...
        if L != 0:
            if L == 1: S = 0
            elif L < 0.5: S = int(round(s * v / (L * 2)))
            else: S = int(round(s * v / (2 - L * 2)))
    ...
    def percentForm(self) -> tuple:
        ...
        h = round((self.hue / 360), 2)
        s = round((self.saturation / 100), 2)
        v = round((self.value / 100), 2)
...
class HSL:
    ...
    def hsv(self) -> tuple:
        ...
        L = (self.lightness / 100) * 2

        if L <= 1: S = (self.saturation / 100) * L
        else: S = 2 - L
...
class YCC:
    ...
    def percentForm(self) -> tuple:
        ...
        y = round((self.cyan / 255), 2)
        cb = round((self.magenta / 255), 2)
        cr = round((self.yellow / 255), 2)
...
class CMYK:
    ...
    def rgb(self) -> tuple:
        ...
        K = self.key / 100
        red = int(round(-255 * (((self.cyan / 100) * (1 - K)) - 1 + K)))
        green = int(round(-255 * (((self.magenta / 100) * (1 - K)) - 1 + K)))
        blue = int(round(-255 * (((self.yellow / 100) * (1 - K)) - 1 + K)))
    ...
    def percentForm(self) -> tuple:
        ...
        c = round((self.cyan / 100), 2)
        m = round((self.magenta / 100), 2)
        y = round((self.yellow / 100), 2)
        k = round((self.key / 100), 2)
